class Door:
    ...
    den Zuständen door_is_open (für geöffnete Türe) sowie
    door_is_locked (für verriegelte Türe).
    Die Türe überwacht die beiden Zustände und verhindert so Aktionen,
    die nicht zulässig sind. Das Verriegeln delegiert die Türe an ein
    Objekt vom Typ Door_lock (Türschloss).
    """

    # Mit dem Keyword def wird eine Funktion bzw. eben ein Konstruktor deklariert.
    # Der Konstruktor heisst IMMER __init__ und weist als ersten Parameter den Wert self auf.
